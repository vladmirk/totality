...
public class HomeController {
    ...
    private List<GameBet> getGameBetList(List<Game> games, UserTournament userTournament) {
        ...
        int userCount = gameService.countUserTournaments(userTournament.getTournament());
        Map<Game, Long> gameMap = accService.countBetsPerTournament(userTournament.getTournament());

        for (Game game : games) {
            Bet bet = accService.findOrCreateBet(game, userTournament);
            BetResultItem betResultItem = accService.findBetResultItem(bet);
            gameBets.add(new GameBet(bet, game, betResultItem, new GameBetStatistics(userCount, gameMap.get(game))));
        }
