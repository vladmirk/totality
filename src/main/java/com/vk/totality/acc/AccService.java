...
public class AccService {
    ...
    @Autowired
    public AccService(BetRepository betRepository, AccountRepository accountRepository, BetResultRepository betResultRepository, GameService gameService, BetResultItemRepository betResultItemRepository) {
    ...
    private void calcGameOutcome(List<BetResultItem> resultItems, BetResult betResult) {
        ...
        for (BetResultStatus status : BetResultStatus.winStatuses()) {
            ...
            if ((items != null) && !items.isEmpty()) {
                BigDecimal winAmount = gameBudget.multiply(status.ratio()).setScale(0, RoundingMode.FLOOR).divide(BigDecimal.valueOf(items.size()), RoundingMode.FLOOR).setScale(0);
